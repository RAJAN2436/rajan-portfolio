...
function MarqueeItem({
  ...
  const itemOffset = useTransform(baseOffset, (v: number) => {
    const space = gap || (100 / itemsLength)
    const position = itemIndex * space
    const totalLength = itemsLength * space
    const wrappedValue = wrap(0, totalLength, v + position)
    const displayValue = easing && wrappedValue <= 100 ? easing(wrappedValue / 100) * 100 : wrappedValue
    return `${displayValue}%`
  })
  ...
  const cssVariables = React.useMemo(() => {
    return Object.fromEntries(
      (cssVariableInterpolation || []).map(({ property }) => [
        property,
        motionValue(0),
      ])
    )
  ...
  useEffect(() => {
    const unsubscribe = itemOffset.on("change", (value: string) => {
      ...
      if (match && match[1]) {
        ...
        currentOffsetDistance.set(parsed);

        // Interpolate and set CSS variables manually to avoid hook violations
        (cssVariableInterpolation || []).forEach(({ property, from, to }) => {
          const fromNum = typeof from === "number" ? from : parseFloat(from as string) || 0
          const toNum = typeof to === "number" ? to : parseFloat(to as string) || 0
          const interpolated = fromNum + (toNum - fromNum) * (parsed / 100)
          
          if (cssVariables[property]) {
            (cssVariables[property] as any).set(interpolated)
          }
        })
      }
    })
    return unsubscribe
  }, [itemOffset, currentOffsetDistance, cssVariableInterpolation, cssVariables])
